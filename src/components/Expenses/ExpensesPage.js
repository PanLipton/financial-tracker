...
import ReceiptWindow from '../Expenses/ReceiptWindow';
import ExpensesGraph from '../Expenses/ExpensesGraph';

// Styled components for the Expenses page
...
const ExpensesEntry = styled.div`
  background: #f0f4f8;
  padding: 1rem;
  margin: 0.5rem 0;
  border-radius: 8px;
...
const ExpensesPage = () => {
  ...
  const [expensesData, setExpensesData] = useState([
    { id: 1, source: 'Clotch', amount: 5000, date: '2023-04-01' },
    { id: 2, source: 'Internet', amount: 1500, date: '2023-04-15' },
    { id: 3, source: 'Foods', amount: 300, date: '2023-04-30' },
  ]);
  ...
  const handleDeleteExpenses = (id) => {
    setExpensesData((prevData) => prevData.filter((entry) => entry.id !== id));
  };

  const ExpensesGraphData = expensesData.map(entry => ({
    date: entry.date,
    amount: entry.amount,
  }));
  ...
  return (
    <ExpenseContainer>
      ...
      <ExpensesGraph data={ExpensesGraphData} />
